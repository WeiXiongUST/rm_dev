# Copyright 2024 RLHFlow team and/or its affiliates
# 
...
class ScriptArguments:
    ...
    # for 8 GPU, the global batch size is 512 = per_device_train_batch_size * gradient_accumulation_steps * 8
    ...
    padding_mode: Optional[str] = field(
        default='add_pad',
        metadata={"help": "If the model has no official padding token, we need to add a padding token"},
    )
...
# Set the padding token for the model, adjusted according to the base model
tokenizer.truncation_side = "left"
tokenizer.model_max_length = script_args.max_length

if not tokenizer.pad_token:
    print("No padding token...")
    if script_args.padding_mode == 'eos':
        tokenizer.pad_token = tokenizer.eos_token
        tokenizer.pad_token_id = tokenizer.eos_token_id
    elif script_args.padding_mode == 'add_pad':
...
def build_dataset(tokenizer, train_path, eval_path):
    ...
    def tokenize(sample):
        sample['positive'] = tokenizer.apply_chat_template(
            sample['chosen'], tokenize=False, add_generation_prompt=False, add_special_tokens=False)
        sample['negative'] = tokenizer.apply_chat_template(
            sample['rejected'], tokenize=False, add_generation_prompt=False, add_special_tokens=False)
    ...
    eval_dataset = load_dataset(eval_path, split="train").shuffle(seed=42)
